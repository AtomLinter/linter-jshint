...
module.exports = {
  ...
  activate() {
    ...
    this.subscriptions.add(atom.config.observe('linter-jshint.disableWhenNoJshintrcFileInPath', disableWhenNoJshintrcFileInPath => {
    ...
    this.subscriptions.add(atom.config.observe('linter-jshint.lintJSXFiles', lintInlineJavaScript => {
  ...
  provideLinter(): Linter$Provider {
    ...
    return {
      ...
      lint: async (textEditor) => {
        ...
        const result = await Helpers.execNode(this.executablePath, parameters, {stdin: fileContents})
        let parsed
        try {
        ...
        for (const entry of parsed.result) {
          ...
          if (error.character == null) {
            ...
            // Todo: Remove workaround of jshint/jshint#1457
          ...
          results.push({
            type: error.code.substr(0, 1) === 'E' ? 'Error' : 'Warning',
